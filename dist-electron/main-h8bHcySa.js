...
import require$$8, { randomFillSync, randomUUID, createHash, randomBytes } from "crypto";
...
var xmlEscape = { exports: {} };
var hasRequiredXmlEscape;
function requireXmlEscape() {
  if (hasRequiredXmlEscape) return xmlEscape.exports;
  hasRequiredXmlEscape = 1;
  var escape2 = xmlEscape.exports = function escape3(string, ignore) {
    var pattern;
  ...
  return xmlEscape.exports;
}
var xmlEscapeExports = requireXmlEscape();
const escape = /* @__PURE__ */ getDefaultExportFromCjs(xmlEscapeExports);
var node$1;
...
const WebSocket2 = /* @__PURE__ */ getDefaultExportFromCjs(nodeExports);
...
function requireForm_data() {
  ...
  FormData2.prototype.submit = function(params, cb) {
    ...
    if (options.protocol === "https:") {
      request = https$12.request(options);
    } else {
      request = http$12.request(options);
    }
...
const HttpsProxyAgent$1 = /* @__PURE__ */ getDefaultExportFromCjs(distExports);
...
function requireFollowRedirects() {
  ...
  followRedirects$1.exports = wrap({ http: http$12, https: https$12 });
...
function getTunnelingAgent(agentOptions, userHttpsAgent) {
  ...
  agent2 = new HttpsProxyAgent$1(merged);
...
function setProxy(options, configProxy, location, isRedirect, configHttpsAgent, configHttpAgent, allowEnvProxy = true) {
  ...
  if (proxy) {
    ...
    if (targetIsHttps) {
      if (!(configHttpsAgent instanceof HttpsProxyAgent$1)) {
...
var Communicate = class {
  ...
  async *_stream() {
    ...
    if (this.proxy) {
      if (!HttpsProxyAgent) {
        try {
          const proxyModule = await import("./index-CQ5QLbKT.js").then((n) => n.i);
      ...
      if (HttpsProxyAgent) {
        agent2 = new HttpsProxyAgent(this.proxy);
      }
    }
  ...
  async *stream() {
    ...
    for (const partialText of this.texts) {
      ...
      try {
        ...
        if (e instanceof AxiosError2 && ((_a = e.response) == null ? void 0 : _a.status) === 403) {
...
function fetchHttpBuffer(urlStr) {
  return new Promise((resolve2, reject) => {
    const client = urlStr.startsWith("https") ? https$1 : http$1;
    client.get(
      urlStr,
      {
        headers: {
...
async function fallbackTTS(text, voice) {
  try {
    ...
    const estimatedDuration = Math.max(3, fullBuffer.length / 3800);
    const words = [];
...
function setupIpcHandlers() {
      ...
      try {
        ...
        for await (const rawChunk of comm.stream()) {
          ...
          if (chunk.type === "audio" && chunk.data) {
            ...
            const start = Number(((chunk.offset || 0) / 1e7).toFixed(2));
            const dur = Number(((chunk.duration || 0) / 1e7).toFixed(2));
            words.push({
        ...
        console.warn("Edge-TTS direct synthesis error, falling back:", (err == null ? void 0 : err.message) || err);
  ...
  ipcMain.handle("render:video", async (_, { project, resolution = "1080p" }) => {
    try {
      ...
      const safeTitle = (project.title || "Video").replace(/[^a-zA-Z0-9_\u00C0-\u024F\u1EA0-\u1EF9]/g, "_").slice(0, 40);
      ...
      const totalSec = project.scenes.reduce(
        (acc, s) => acc + Math.max(s.audioDuration || 4, 2),
        0
      );
      const durationInFrames = Math.max(Math.round(totalSec * fps), 30);
